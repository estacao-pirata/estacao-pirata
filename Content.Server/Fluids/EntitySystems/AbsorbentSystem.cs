...
public sealed class AbsorbentSystem : SharedAbsorbentSystem
{
    ...
    private bool TryTransferAbsorber(EntityUid user, EntityUid used, EntityUid target, AbsorbentComponent component, Solution absorberSoln)
    {
        ...
        FixedPoint2 totalEvaporationReagent = 0;
        foreach (var evaporationReagent in PuddleSystem.EvaporationReagents)
        {
            totalEvaporationReagent += absorberSoln.GetReagentQuantity(evaporationReagent);
        }

        var transferAmount = component.PickupAmount / PuddleSystem.EvaporationReagentRatio -
            totalEvaporationReagent;
        ...
        if (refillableSolution.AvailableVolume == FixedPoint2.Zero)
        {
            ...
            DebugTools.Assert(water.Volume == FixedPoint2.Zero);
            return false;
        }
        ...
        var nonWater = absorberSoln.SplitSolutionWithout(nonWaterAmount, PuddleSystem.EvaporationReagents);
    ...
    private bool TryPuddleInteract(EntityUid user, EntityUid used, EntityUid target, AbsorbentComponent absorber, Solution absorberSoln)
    {
        ...
        var absorberSplit = absorberSoln.SplitSolutionWithOnly(puddleSplit.Volume / PuddleSystem.EvaporationReagentRatio, PuddleSystem.EvaporationReagents);
        ...
        absorberSoln.AddSolution(puddleSplit.SplitSolution(absorberSoln.AvailableVolume), _prototype);
